...
def configure_run_on_startup(enable: bool, flags: set):
    ...
    if platform.system() == "Windows":
        ...
        if enable:
            ...
            <dict>
                ...
                <array>
                    <string>{str(executable_path)}</string>{ ''.join(f'''
                    <string>{flag}</string>''' for flag in flags) }
                </array>
                <key>RunAtLoad</key>
                <true/>
            </dict>
        ...
        if enable:
            ...
            [Desktop Entry]
            Type=Application
            Exec="{str(executable_path)}" {' '.join(flags)}
            Hidden=false
            NoDisplay=false
