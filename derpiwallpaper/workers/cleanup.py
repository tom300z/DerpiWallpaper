...
from derpiwallpaper.config import CLEANUP_INTERVAL, CONFIG
...
class WallpaperCleanupWorker(WorkerThread):
    ...
    def _perform_cleanup(self):
        ...
        files = [f for f in CONFIG.wallpaper_folder.glob("derpibooru_*") if f.is_file()]

        # Sort files by modification time (most recent last)
        files.sort(key=lambda f: f.stat().st_mtime)

        # Check if we have more files than we want to keep
        if len(files) > CONFIG.wallpapers_to_keep:
            files_to_delete = files[:-CONFIG.wallpapers_to_keep]  # All but the most recent ones
